...
# --- Sidebar Sequential Radio Navigation & User Guide ---
...
st.sidebar.markdown("---")
st.sidebar.subheader("📖 Quick User Guide")

with st.sidebar.expander("How to Use This App"):
  st.markdown("""
    **1. Browse Available Books**
    * Go to **Browse available books** from the main menu.
    * Use filters (Department, Semester) or search by title/author.
    * Click **📱 WhatsApp QR** to scan and chat with the seller instantly.
    
    **2. List Your Old Book**
    * Select **List a book for sale**.
    * Fill out the book details, price, and your contact info.
    * *Important:* Set a **4-digit PIN** and a **Secret Recovery Word** so you can manage or edit your listing later.
    
    **3. Edit or Mark as Sold**
    * Under **Browse available books**, click **Mark as Sold** or **Edit Listing** on your post.
    * Enter your 4-digit PIN to confirm.
    * Forgot your PIN? Click **🔄 Forgot PIN?** and use your Secret Recovery Word to generate a new one.
    
    **4. Ask the Assistant**
    * Head over to the **FAQ Chatbot** tab if you have questions about available stock or app features!
    """)
  ...
  if user_query := st.chat_input(
    ...
    if is_inventory_query:
      try:
        ...
        matched_books = pd.DataFrame(columns=available_books.columns)
        stop_words = {
            "do",
            "you",
            "have",
        ...
        if matched_books.empty:
          bot_response = (
              "I checked our active inventory, but I couldn't find any books"
              " matching your request right now. You can check the **'Browse"
              " available books'** page to see all active listings!"
          )
        else:
          bot_response = (
    ...
    if not bot_response:
      if any(
        ...
        bot_response = (
            ...
            " **recovering your PIN**, **using WhatsApp QR codes**, or"
